...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    return LabeledPoint(float(line.split(',')[0]), [float(w) for w in line.split(',')[1:]])
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return labelsAndPreds.map(lambda (x,y): squaredError(x,y)).mean()**0.5
...
def twoWayInteractions(lp):
    ...
    """
    return LabeledPoint(lp.label, np.hstack((lp.features, [x*y for (x,y) in list(itertools.product(lp.features, lp.features))])))

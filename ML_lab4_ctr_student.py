...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    return SparseVector(numOHEFeats, [(OHEDict[t], 1) for t in rawFeats])
...
def parsePoint(point):
    ...
    """
    return [(i,point.split(",")[i + 1]) for i in range(len(point.split(",")) - 1)]
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    return SparseVector(numOHEFeats, [(OHEDict[t], 1) for t in rawFeats if t in OHEDict])

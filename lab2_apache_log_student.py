...
dayHostCount = dayGroupedHosts.map(lambda (k, v): (k, len(list(set(v)))))

dailyHosts = dayHostCount.sortByKey(True).cache()
...
avgDailyReqPerHost = (sortedByDay.map(lambda (x,y): (x, len(y)/len(list(set(y)))))).cache()
...
errDateSorted = (errDateSum.sortByKey(True).cache())
...
hourRecordsSorted = (hourRecordsSum.sortByKey(True).cache())

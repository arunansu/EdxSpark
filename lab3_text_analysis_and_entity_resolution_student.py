...
def tf(tokens):
    ...
    tokenPair = {}
    for token in tokens:
        if tokenPair.has_key(token):
            tokenPair[token] = tokenPair[token] + 1.0/tokensTotal
        else: 
            tokenPair[token] = 1.0/tokensTotal
    return tokenPair
...
def dotprod(a, b):
    ...
    for k in a:
        if b.has_key(k):
            dotprodtotal = dotprodtotal + a[k] * b[k]
    return dotprodtotal
...
trueDupsRDD = (sims
               .join(goldStandard)).map(lambda s: s[1][0])
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.mean()
nonDupsRDD = (sims
              .subtractByKey(goldStandard)).map(lambda s: s[1])
...
def swap(record):
    ...
    Returns:
        pair: ((ID, URL), token)
    """
    return [((k,v), record[0]) for (k,v) in record[1]]

commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD)
                .groupByKey()
                .flatMap(lambda x: swap(x))
                .groupByKey()
                .cache())
...
def fastCosineSimilarity(record):
    ...
    s = 0
    for t in record[1]:
        s = s + amazonWeightsBroadcast.value[amazonRec][t] * googleWeightsBroadcast.value[googleRec][t]

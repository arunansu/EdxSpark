...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda(x,y) : math.pow((y[0] - y[1]), 2)))
...
# TODO: Replace <FILL IN> with appropriate code
myModel = ALS.train(trainingRDD, 8, seed=5L, iterations=5, lambda_=regularizationParameter)
...
# TODO: Replace <FILL IN> with appropriate code
myRatingsModel = ALS.train(trainingWithMyRatingsRDD, bestRank, seed=5L, iterations=5, lambda_=regularizationParameter)
...
testRMSEMyRatings = computeError(predictedTestMyRatingsRDD, testForAvgRDD)
...
myUnratedMoviesRDD = (moviesRDD
                      .map(lambda(x,y): (myUserID, x)).filter(lambda (x,y): y not in myRatedMovies))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD).map(lambda (x,y): (y[0][0], y[1], y[0][1])))
